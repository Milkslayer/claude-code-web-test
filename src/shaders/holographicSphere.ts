export const holographicVertexShader = `
  ...
  void main() {
    ...
    // Create displacement based on noise and time
    float noise = snoise(position * 2.0 + time * 0.5);
    vDisplacement = noise * displacement;
...
export const holographicFragmentShader = `
  ...
  void main() {
    ...
    vec3 color = mix(primaryColor, secondaryColor, vDisplacement * 0.5 + 0.5);

    // Add glow and scanlines
    color = color * (0.5 + scanline * 0.5);
    color += color * fresnel * glowIntensity;
